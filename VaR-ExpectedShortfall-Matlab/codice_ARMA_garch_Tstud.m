...
% Specifica il modello ARMA(1,1)-GARCH(1,1)
Mdl = arima('Constant', 0, 'ARLags', 1, 'MALags', 1, 'Variance', garch(1,1));

% Stima dei parametri del modello ARMA(1,1)-GARCH(1,1)
EstMdl = estimate(Mdl, logReturn);

% Visualizzazione dei risultati della stima
disp(EstMdl);

% Verifica dell'adattamento del modello ARMA-GARCH ai dati iniziali
...
[h_arma_garch, p_arma_garch] = lbqtest(residui_standardizzati);

disp('Test di Adattamento del Modello ARMA-GARCH ai Dati Iniziali:');
disp(['h = ', num2str(h_arma_garch), ', p = ', num2str(p_arma_garch)]);
...
title('Residui Standardizzati del Modello ARMA-GARCH');
xlabel('Giorni');
